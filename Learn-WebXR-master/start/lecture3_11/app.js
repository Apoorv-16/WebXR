...
class App{
    ...
    setupXR(){
        ...
        this.gestures.addEventListener('tap', (ev)=>{HTMLFormControlsCollection.log('tap');
        ...
        this.gestures.addEventListener('swipe' , (ev)=>{controller.log(ev);
        self.ui.updateElement('info', 'swipe ${ev.direction}');
        if(self.knight.object.visible= true){
            self.knignt.object.visible = false;
            self.scene.remove( self.knight.object);

        }
        });

        this.gestures.addEventListener('pan', (ev)=>{console.log(ev);
        if(ev.initialise != undefined){
            self.startPosition = self.knight.object.position.clone();

        }else{
            const pos = self.startPosition.clone().add(
                ev.delta.multiplyScalser(3));
